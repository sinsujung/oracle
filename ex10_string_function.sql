...
SELECT
	ADDRESS,
	substr(address, 3, 5), --sql은 인덱스 1부터 시작
	substr(address, 3)
FROM TBLADDRESSBOOK;
...
SELECT
	...
	instr('안녕하세요. 홍길동님. 홍길동님', '홍길동', -1) AS r6 -- 역순으로 찾는 용도(-1)
FROM dual;
...
SELECT
	lpad('a', 5), --%5s
	lpad('a', 5, 'b'), --a로 자릿수를 채우고 나머지는 b로 채워주세요~
	lpad('aaa', 5, 'b'),
	lpad('aaaa', 5, 'b'),
	lpad('aaaaa', 5, 'b'), --나머지 채울 공간 없어서 b는 의미가 없음~
	lpad('1', 3, '0'),
	rpad('1', 3, '0')
FROM dual;
...
/*
	...
	- decode()
	- replace()와 유사
	- varchar2 decode(컬럼, 찾을 문자열, 바꿀 문자열, 찾을 문자열, 바꿀 문자열, [찾을 문자열, 바꿀 문자열] x N)
...
SELECT
	...
	END AS 성별,
	replace(gender, 'm', '남자') AS 남자,
	replace(replace(gender, 'm', '남자'), 'f', '여자') AS 여자,
	decode(gender, 'm', '남자', 'f', '여자') AS 성별2
FROM tblComedian;
...
SELECT
	...
	END) AS 여자1,
	count(DECODE(gender, 'm', 1)) AS 남자2,
	count(DECODE(gender, 'f', 1)) AS 여자2
FROM TBLCOMEDIAN;
